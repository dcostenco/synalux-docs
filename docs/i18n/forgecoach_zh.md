...
*   **AI 生成计划**（Elite）— 描述您的目标与限制，ForgeCoach 使用 Prism 8B 或 Claude Sonnet 生成完全定制的多周训练块。
...
*   **ForgeCoachCore** — 包含所有引擎、模型和业务逻辑的共享 Swift 包（SPM），由 iOS 应用、Mac Catalyst 应用和 watchOS 扩展使用。
...
*   **订阅验证** — 通过 HTTPS 向 `api.forgecoach.app/subscription/check` 发送邮箱地址，邮箱以 24 小时 TTL 和 48 小时宽限期存储于 Keychain。
...
在 [forgecoach.app/subscribe](https://forgecoach.app/subscribe) 订阅 — Stripe 支持的网页结账，无需应用内购买。
