...
*   **免费版** — 无 AI（仅静态计划模板）。
*   **Pro — 设备端 Prism 1.7B** — 通过带 Metal 加速的 llama.cpp 运行。零网络，完全私密。当可用 RAM ≥ 1,600 MB 时使用 Prism Coder 1.7B（设备端，Metal）；当可用 RAM 为 450–799 MB 时使用 SmolLM2-360M（185 MB，设备端）；指导从不中断，只会逐级降级。回答您关于训练的问题，建议动作纠正，根据准备度调整今日训练量。
...
*   **6 个 Watch 标签** — 仪表板（身体电量）、肌肉地图、训练记录、Watch AI教练、中枢神经系统点击测试、设置。
*   **Watch AI教练** — 手腕上的完整对话式 AI：查询通过 WatchConnectivity 中继至配对 iPhone；iPhone 在设备端运行 1.7B → 360M → 云端级联并将回复流式返回；iPhone 无法连接时自动切换至明确的"iPhone 不可达"状态。
