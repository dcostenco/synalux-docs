...
*   **AI 생성 프로그램** (Elite) — 목표와 제약 조건을 설명하면 ForgeCoach가 Prism 8B 또는 Claude Sonnet을 사용하여 완전 맞춤형 다주간 블록을 생성.
...
*   **Pro — 온디바이스 Prism 1.7B** — Metal 가속이 적용된 llama.cpp를 통해 실행. 네트워크 없음, 완전 비공개. 트레이닝에 관한 질문에 답변하고, 자세 수정을 제안하며, 준비 상태에 따라 오늘의 볼륨 조정.
...
*   **5개의 Watch 탭** — 대시보드 (바디 배터리), 근육 지도, 운동 기록, CNS 탭 테스트, 설정.
...
*   **ForgeCoachCore** — 모든 엔진, 모델, 비즈니스 로직이 포함된 공유 Swift 패키지(SPM). iOS 앱, Mac Catalyst 앱, watchOS 익스텐션에서 사용.
...
*   **구독 확인** — HTTPS를 통해 `api.forgecoach.app/subscription/check`에 이메일 전송. 이메일은 24시간 TTL과 48시간 유예 기간으로 Keychain에 저장.
...
[forgecoach.app/subscribe](https://forgecoach.app/subscribe)에서 구독 — Stripe 기반 웹 결제. 인앱 구매 불필요.
