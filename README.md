...
```bash
bash scripts/git-hooks/install.sh   # auto-sync skills on commit & pull
```

Skills in `skills/` auto-sync to the local Prism DB on every commit and `git pull`. No manual steps after install.

### Monitoring & Analytics

Real-time Datadog RUM dashboards (sessions, errors, actions, modules, devices, countries, billing plans):

| App | Dashboard | Service |
|-----|-----------|---------|
| **Synalux Portal** | [Module Analytics](https://app.datadoghq.com/dashboard/8ty-t7r-hey/synalux-portal--module-analytics) | `synalux-portal` |
